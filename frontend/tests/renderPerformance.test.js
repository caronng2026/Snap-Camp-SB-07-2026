 ...
 *
 * Under PRD-002 the entries arrive from the server, so these figures now include an
 * already-resolved promise hop from the in-memory API double. They still measure the
 * app's own render cost, not network time — network cost is measured separately.
 *
...
function seed(entryCount, distinctSkus) {
  ...
  for (let i = 0; i < entryCount; i += 1) {
    entries.push({
      sku: String(1000 + (i % distinctSkus)),
      quantity: (i % 9) + 1,
      recordedAt: new Date(2026, 7, 4, 9, 0, i % 60).toISOString(),
    })
  }
  api = createMemoryApi({ days: { [day]: entries } })
}
 ...
 */
async function timeRender(passes = 7) {
  const samples = []
  for (let i = 0; i < passes; i += 1) samples.push(await mountAndTime())
  samples.sort((a, b) => a - b)
  return samples[Math.floor(samples.length / 2)]
}

describe('render cost at scale — PRD-001-NFR03', () => {
  it('renders a 200-entry day', async () => {
    seed(200, 60)
    const ms = await timeRender()
  ...
  it('renders a 1,000-entry day, matching the anchor partner\'s catalogue size', async () => {
    seed(1000, 300)
    const ms = await timeRender()
  ...
  it('stays responsive when an entry is added to a large day', async () => {
    seed(200, 60)
    await timeRender(3)
    const started = performance.now()
