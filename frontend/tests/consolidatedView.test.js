...
describe('consolidated totals are visible in the log — PRD-001-UX03', () => {
  it('shows one row with the summed total when a SKU repeats', () => {
    record('734', '3')
    record('734', '2')
  ...
  it('shows how many entries make up a row, so the merge is not taken on trust', () => {
    record('734', '1')
    record('734', '1')
    record('734', '1')
  ...
  it('keeps different SKUs on separate rows', () => {
    record('734', '3')
    record('91', '1')
    record('734', '2')
  ...
  it('updates the total in place as the same SKU is entered again', () => {
    record('734', '3')
...
describe('consolidation is derived, never stored — PRD-001-FR04', () => {
  it('leaves the original entries intact in storage', () => {
    record('734', '3')
    record('734', '2')
    const stored = entriesFor(loadState(store).log, dateKeyFor())
    expect(stored).toHaveLength(2)
    expect(stored.map((e) => e.quantity)).toEqual([3, 2])
  })

  it('still consolidates correctly after a reload', () => {
    record('734', '3')
    record('734', '2')
    record('91', '1')
    reload()
  ...
  it('shows the empty state when a day has no entries', () => {
