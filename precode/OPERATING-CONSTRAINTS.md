...
- Start Precode-driven agent sessions with `cd precode && ...` from the repository root. The agent shims (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`) are only discovered when the working directory is inside `precode/`, not from the repository root.
