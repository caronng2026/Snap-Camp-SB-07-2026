...
| OQ-12 | Should the export stay `.xlsx`, or revert to `.csv`? | `PRD-001-FR05`, `NFR02`; the export bead | **Open — blocks the export bead.** `.xlsx` was chosen solely to preserve leading zeros. With zeros now stripped (2026-07-31), that reason is gone and `.csv` would drop a dependency and simplify the bead. `.xlsx` still offers native workbook formatting. Decide before the export bead starts. |
